...
[data-theme="dark"] {
    ...
    --bg3: #162030;
    --bg4: #1d2d42;
    --line: rgb(178 162 162 / 62%);
    --line2: rgb(255 241 241);
    --text: #e8edf5;
    --text2: #fefefe;
    --text3: #dce8f7;
    --nav-bg: #0b1220;
...
.tna-node:not(:last-child)::after {
  content:'';position:absolute;top:100%;left:12.5px;
...
/* Chart */
.bar-row { display:flex;align-items:flex-end;gap:8px;height:100px;padding-top:8px; }
.bar-col { 
  /* flex:1; */
  display:flex;flex-direction:column;align-items:flex-start;gap:4px; }
.bar-body {
  width:45px;border-radius:4px 4px 0 0;
  min-height:4px; max-height:90px; transition:height .8s cubic-bezier(.22,1,.36,1);
}
...
async function renderHeat(seas, buy, comp){
  ...
  try{
    ...
    const resp = await fetch(`<?php echo base_url()?>Dashboard/getheatmap?seas=${encodeURIComponent(JSON.stringify(seas))}&buy=${encodeURIComponent(JSON.stringify(buy))}&comp=${encodeURIComponent(JSON.stringify(comp))}`);
